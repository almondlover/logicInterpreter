...
namespace courseprojprotonew
{
    ...
    internal abstract class TreeNode
    {
        ...
        public void Copy(TreeNode copy)
        {
            if (copy.Token == null || copy == null) copy = new BranchNode(new Token(this.Token.Type)); 
            if (this.Token.Type == TokenType.Operand) { copy=new LeafNode(new Token(TokenType.Operand)); return; }
            else for (int i = 0; i < ((BranchNode)this).operands.Count; i++)
                {
                    ((BranchNode)copy).operands.Add(new BranchNode(new Token(((BranchNode)this).operands[i].Token.Type)));
                    ((BranchNode)this).operands[i].Copy(((BranchNode)copy).operands[i]);    
                }
        }
    }
    ...
    internal class Program
    {
                    ...
                    case TokenType.Or:
                        {
                            ...
                            else
                            {
                                while (treeStack.Count > 0)
                                {
                                    ...
                                    current = (BranchNode)treeStack.Pop();
                                }
        ...
        static void ReadCommand()
        {
            ...
            while (true)
            {
                ...
                switch (curCommand)
                {
                    ...
                    case "SOLVE": { SolveFunction(s, out success); } break;
                    case "ALL": { GetTable(s, ref success); } break;
        ...
        static void Define(string input)
        {
            bool success;
            string funcName="";
            int[,] argidx = null;
            ...
            if (!success) { Console.WriteLine("Syntax Error!"); return; }
            else Console.WriteLine("New function added.");
            var tree=BuildTree();
        ...
        static void DisplayFunctionBody(TreeNode root, char[] names, ref int ind, ref string output)
        {
        ...
        static void SolveFunction(string input, /*string name, bool[] values, ref bool result,*/ out bool success)
        {
            success = true;
            bool result;
            ...
            if (!success) { Console.WriteLine("Incorrect syntax!"); return; }
            
            //success = true;
            
            Tuple<int[,], TreeNode> pair;
            if (!functions.TryGetValue(name, out pair)) { success = false;  return;}
        ...
        static void SolveSubtree(TreeNode root, TreeNode duplicate, bool[] curvalues, bool[] prevalues, ref List<bool> nodeResults, ref int n, int startind, int endind, bool resume)
        {
            int i = 0;
            if (root.Token.Type == TokenType.Operand) return;

            //cikal        
            for (i = 0; i < ((BranchNode)root).operands.Count /*&& ind < curvalues.Length*/; i++)
            {
                if (resume) { n++; continue; }
                if (prevalues==null||prevalues==curvalues) { nodeResults.Add(root.Value); n++; SolveSubtree(((BranchNode)root).operands[i], ((BranchNode)duplicate).operands[i], curvalues, prevalues, ref nodeResults, ref n, startind, endind, false) ; }
                //int ind1 = ind;
                startind = endind;
                SetLeafNodeValues(((BranchNode)root).operands[i], curvalues, ref endind);
                int ind = startind;
                //ind = ind1;
                bool changed = false;
                while (ind++ <= endind && !changed) changed = prevalues[ind] ^ curvalues[ind];
                    if (changed) { nodeResults[n]=root.Value; n++; SolveSubtree(((BranchNode)root).operands[i], ((BranchNode)duplicate).operands[i], curvalues, prevalues, ref nodeResults, ref n, startind, endind, false); }
                    else { ((BranchNode)duplicate).operands[i]=new LeafNode(null); SolveSubtree(((BranchNode)root).operands[i], duplicate, curvalues, prevalues, ref nodeResults, ref n, startind, endind, true); }

            }
            //result = duplicate.Value;
        }
        static void SetSubtreeValues(TreeNode root, bool[] curvalues, bool[] prevalues, ref List<bool> nodeResults, ref List<int> rNodeIdx, int ind, int n)
        {
            if (root.Token.Type == TokenType.Operand) return;
            //cikal        
            for (int i = 0; i < ((BranchNode)root).operands.Count /*&& ind < curvalues.Length*/; i++, n++)
            {

                if (prevalues == null || prevalues == curvalues) { nodeResults.Add(root.Value);
                    SetSubtreeValues(((BranchNode)root).operands[i], curvalues, prevalues, ref nodeResults, ref rNodeIdx, ind, n);
                    if (i == 0 && ((BranchNode)root).operands.Count == 2) rNodeIdx.Add(nodeResults.Count); else rNodeIdx.Add(n);
                    continue;
                }
                ...
                int endind = ind;
                if (i == 0) ind = startind;
                while (startind++ <= endind && !changed) changed = prevalues[endind] ^ curvalues[endind];
                if (changed) nodeResults[n] = root.Value;
                else { ((BranchNode)root).operands[i] = new LeafNode(new Token(TokenType.Operand)); n = rNodeIdx[n] + 1; }
                SetSubtreeValues(((BranchNode)root).operands[i], curvalues, prevalues, ref nodeResults, ref rNodeIdx, ind, n);

            }
            //result = duplicate.Value;
        }
        static void GetTable(string input, /*TreeNode root, bool[]curvalues, ref int ind, ref bool[] results,*/ ref bool success)
        {
            string funcName = "";
            int i = 0;
            if (input.Length == 0) { success = false; return; }
            ...
            while (i < input.Length) { if (input[++i] != ' ') { success = false; return; } }
            Tuple<int[,], TreeNode> pair;
            if (!functions.TryGetValue(funcName, out pair)) { success = false; return; }
       ...
       //sh sa gramna
        ...
        static void FindFunction(bool[][] values, bool[] results, ref TreeNode found)
        {
            ...
            if (!success) { Console.WriteLine("No function found!");}
